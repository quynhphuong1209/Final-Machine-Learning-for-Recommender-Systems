...
for i, (uid, items) in enumerate(top_n.items()):
    ...
    html_tables.append(f"""
        ...
        ">
            <h4 style="margin-bottom: 10px; font-family: Arial; color: white;">
                👤 Bệnh nhân {uid}
            </h4>
            {table_html}
        </div>
...
# Hiển thị 3 bảng theo hàng ngang, căn giữa, cách đều
final_html = f"""
<div style="
    display: flex;
    flex-direction: row;
    ...
    gap: 30px;
    background-color: #222;  /* nền tối để chữ trắng nổi bật */
    padding: 20px;
    border-radius: 10px;
">
    {''.join(html_tables)}
